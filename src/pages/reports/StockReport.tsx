...
export default function StockReport() {
  ...
  useEffect(() => { fetchStockData(); }, [selectedMonth, selectedYear]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><ClipboardList className="h-6 w-6" />Stock Report</h1>
          <p className="text-muted-foreground">Comprehensive stock register, summary & movements</p>
        </div>
      ...
      <Tabs defaultValue="register" className="space-y-4">
        <TabsList><TabsTrigger value="register">Stock Register</TabsTrigger><TabsTrigger value="summary">Stock Summary</TabsTrigger><TabsTrigger value="movements">Stock Movements</TabsTrigger><TabsTrigger value="items">Item Details</TabsTrigger></TabsList>

        <div className="flex flex-wrap gap-4 items-center">
          <div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search items..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" /></div>
          <Select value={categoryFilter} onValueChange={setCategoryFilter}><SelectTrigger className="w-[150px]"><SelectValue placeholder="Category" /></SelectTrigger><SelectContent><SelectItem value="all">All Categories</SelectItem>{categories.map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}</SelectContent></Select>
          <Select value={filterType} onValueChange={(v: any) => setFilterType(v)}><SelectTrigger className="w-[140px]"><Filter className="h-4 w-4 mr-2" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Items</SelectItem><SelectItem value="in-stock">In Stock</SelectItem><SelectItem value="out-of-stock">Out of Stock</SelectItem><SelectItem value="low-stock">Low Stock</SelectItem></SelectContent></Select>
        </div>

        <TabsContent value="register">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">Stock Register - {MONTHS.find(m => m.value === selectedMonth)?.label} {selectedYear}</CardTitle>
              <CardDescription>Opening Qty + Purchase Qty - Sale Qty = Closing Qty</CardDescription>
            </CardHeader>
        ...
        <TabsContent value="movements">
          <div className="flex flex-wrap gap-4 items-center mb-4">
            <Select value={movementTypeFilter} onValueChange={setMovementTypeFilter}><SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Types</SelectItem><SelectItem value="purchase">Stock In</SelectItem><SelectItem value="sale">Stock Out</SelectItem></SelectContent></Select>
            <DateRangeFilter dateRange={dateRange} onDateRangeChange={setDateRange} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div className="metric-card"><div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">Total Stock In</p><ArrowUpCircle className="w-4 h-4 text-success" /></div><p className="text-2xl font-bold mt-2 text-success">{totalIn} units</p></div>
            <div className="metric-card"><div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">Total Stock Out</p><ArrowDownCircle className="w-4 h-4 text-destructive" /></div><p className="text-2xl font-bold mt-2 text-destructive">{totalOut} units</p></div>
            <div className="metric-card"><div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">Net Movement</p><Package className="w-4 h-4 text-primary" /></div><p className="text-2xl font-bold mt-2 text-primary">{totalIn - totalOut} units</p></div>
          </div>
          <div className="metric-card overflow-hidden p-0">
            <table className="data-table"><thead><tr><th>Date</th><th>Item</th><th>Type</th><th>Reference</th><th className="text-center">Quantity</th></tr></thead>
              <tbody>{filteredMovements.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">No movements found</td></tr> : filteredMovements.map((m) => (<tr key={m.id}><td className="text-muted-foreground">{m.date ? format(new Date(m.date), 'dd MMM yyyy') : '-'}</td><td className="font-medium">{m.item}</td><td><span className={cn("inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full", m.type === "purchase" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive")}>{m.type === "purchase" ? <ArrowUpCircle className="w-3 h-3" /> : <ArrowDownCircle className="w-3 h-3" />}{m.type === "purchase" ? "Stock In" : "Stock Out"}</span></td><td className="font-medium">{m.reference}</td><td className={cn("text-center font-medium", m.type === "purchase" ? "text-success" : "text-destructive")}>{m.type === "purchase" ? "+" : "-"}{m.qty}</td></tr>))}</tbody>
            </table>
          </div>
        </TabsContent>
        ...
        <TabsContent value="items">
          <div className="metric-card overflow-hidden p-0">
            <table className="data-table"><thead><tr><th>Item Name</th><th>HSN</th><th>Category</th><th className="text-center">Stock</th><th className="text-right">Purchase Rate</th><th className="text-right">Sale Rate</th><th className="text-right">Margin</th></tr></thead>
